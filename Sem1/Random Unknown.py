...
class VisualEnvironment:
    def __init__(self, size=15, obstacle_prob=0.3):
        ...
        # 2. Randomize Goal Position (Make sure it's not on the agent)
        while True:
            gx, gy = random.randint(0, size-1), random.randint(0, size-1)
            if (gx, gy) != self.agent_pos:
                self.goal_pos = (gx, gy)
                break
        ...
        for _ in range(int(size * size * obstacle_prob)):
            ...
            # Ensure obstacle isn't on Start or Goal
            if (ox, oy) != self.agent_pos and (ox, oy) != self.goal_pos:
                self.true_obstacles.add((ox, oy))

        # Setup Plot
        self.fig, self.ax = plt.subplots(figsize=(6, 6))
        self.ax.set_title("Red: Unknown | Black: Known | Green: Path")
        plt.ion() 
    ...
    def update_plot(self, current_path):
        ...
        for obs in self.true_obstacles:
            grid_data[obs[1], obs[0]] = 1 # Red
        for obs in self.known_obstacles:
            grid_data[obs[1], obs[0]] = 2 # Black
        ...
        if current_path:
            ...
            self.ax.plot(px, py, color='green', linewidth=2, alpha=0.7)

        self.ax.plot(self.goal_pos[0], self.goal_pos[1], 'r*', markersize=15, label='Goal')
        self.ax.plot(self.agent_pos[0], self.agent_pos[1], 'bo', markersize=10, label='Agent')
